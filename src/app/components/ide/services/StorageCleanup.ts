...
export class StorageCleanup {
  ...
  static async cleanup(options: CleanupOptions = {}): Promise<CleanupResult> {
    ...
    console.log("[StorageCleanup] Starting cleanup with options:", options);
  ...
  private static async cleanupFiles(
    ...
    try {
      ...
      console.log(`[StorageCleanup] Would clean ${filesToDelete.length} files, freeing ${(spaceFreed / 1024 / 1024).toFixed(2)} MB`);
  ...
  private static async cleanupChatHistory(
    ...
    try {
      ...
      console.log(`[StorageCleanup] Would clean ${sessionsToRemove.length} chat sessions`);
  ...
  private static async cleanupSnapshots(
    ...
    try {
      ...
      console.log(`[StorageCleanup] Would clean ${snapshotsToDelete.length} snapshots`);
  ...
  private static async cleanupEmptyProjects(
    ...
    try {
      ...
      console.log(`[StorageCleanup] Would clean ${emptyProjects.length} empty projects`);
